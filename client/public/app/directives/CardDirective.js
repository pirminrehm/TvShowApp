	...
	return {
		...
		controller: [ '$scope', function( $scope ) {
			...
			function setEpisodeWatched ( card, j ) {
				angular.element("#updateProgress span").html(" ");
				angular.element("#updateProgress span").addClass( "glyphicon glyphicon-time" );
				UserService.setWatched( $scope.token, true, card.episodes[ j ].id )
					.then( function( res ) {
						card.episodes[ j ].w = true;
						angular.element("#updateProgress span").html("+1");
						angular.element("#updateProgress span").removeClass( "glyphicon glyphicon-time" );
					}, function( err ) {
